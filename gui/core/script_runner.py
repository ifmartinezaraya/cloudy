...
class ScriptRunner:
    ...
    def run(self, script_key: str, params: Optional[Dict[str, Any]] = None,
            ...
            timeout: Optional[int] = 300) -> ScriptResult:
    ...
    def run_async(self, script_key: str,
        ...
        """
        def _worker():
            result = self.run(script_key, params, flags, timeout)
            if callback:
                callback(result)
    ...
    def configure_cloud(self, provider: Optional[str] = None, callback=None):
        ...
        if callback:
            return self.run_async("configure", params=params, callback=callback)
        return self.run("configure", params=params)
